...
- `POST /api/alerts` - Create alert (auth optional)
- `POST /api/alerts/:id/upvote` - Upvote alert
- `DELETE /api/alerts/:id` - Delete alert

### Comments
- `GET /api/alerts/:id/comments` - Get comments for alert
- `POST /api/alerts/:id/comments` - Add comment (auth optional)
...
alerts: id, user_id, latitude, longitude, category, description, severity, created_at, expires_at, status, upvotes
comments: id, alert_id, user_id, username, content, created_at
```
